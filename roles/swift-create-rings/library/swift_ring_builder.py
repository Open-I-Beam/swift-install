...
class RingBuilder:
    ...
    def __create__(self):
        ...
        cmd = 'swift-ring-builder %s.builder create %s %s %s'%(type,part_power,replicas,min_part_hours)
    ...
    def __check_params__(self,params):
        ...
        for p in params:
            if (p not in module.params) or (not module.params[p]):
                missing.append(p)
        if len(missing) > 0:
           module.fail_json(msg='Missing required arguments: %s'%(' '.join(missing)))
    ...
    def add(self):
        self.__check_params__(['dev','region','zone','ip','port','weight'])
        if not self.builder_file_exists():
           self.__create__()  
        ...
        cmd = 'swift-ring-builder %s.builder search R%s:%s/%s'%(type,ip,port,dev)
        rc,out,err = module.run_command(cmd , cwd=chdir)
        if 'No matching devices found' in out: 
            #add device to ring
            cmd = 'swift-ring-builder %s.builder add r%sz%s-%s:%s/%s %s'%(type,region,zone,ip,port,dev,weight)
            rc,out,err = module.run_command(cmd , cwd=chdir, check_rc=True)
            module.exit_json(changed=True)
...
def main():
    module = AnsibleModule(
        argument_spec = dict(
            op=dict(required=True, aliases=['operation'], choices=['add','rebalance','create']),
            type=dict(required=True, choices=['container','object','account']),
            chdir=dict(required=False,default='/etc/swift',aliases=['directory']),    
            dev=dict(default=None, aliases=['device']),
            ip=dict(default=None),
            port=dict(default=None),
            region=dict(default=None),
            zone=dict(default=None),
            weight=dict(),
            part_power=dict(default='18'),
            replicas=dict(default='3'),
            min_part_hours=dict(default='1'),
        ),
    )
